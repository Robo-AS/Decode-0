...
public class TurretCR extends SubsystemBase {
    ...
    private void updateHeadings() {
        ...
        if (useLL) {
            ...
            robot.getInstancePinpoint().setPosition(poseFromLL);
            poseSynced = true;
        }
        else if(usePinpoint){
            robot.pinpoint.update();

            double goalX = FIELD_TAGS_LL.get(20).x;
            double goalY = FIELD_TAGS_LL.get(20).y;

            double dx = goalX - robot.pinpoint.getPosX(DistanceUnit.METER);
            double dy = goalY - robot.pinpoint.getPosY(DistanceUnit.METER);

            targetAngle = Math.atan2(dy, dx) - robot.pinpoint.getHeading(AngleUnit.RADIANS) - Math.toRadians(axon.getCurrentAngle());
