...
@Autonomous(name = "🏰IST upper blue ❤️")
public class ISTupperRedAuto extends CommandOpMode {
    ...
    private void buildPaths() {
        preload = follower.pathBuilder().addPath(
                new BezierLine(new Pose(122.99, 124.010), new Pose(84, 84.000))
        ).setLinearHeadingInterpolation(Math.toRadians(36), Math.toRadians(-45)).build();

        spike2 = follower.pathBuilder().addPath(
                new BezierCurve(new Pose(84.000, 84.000), new Pose(78.84, 60.604982206405694), new Pose(126, 59.44))
        ).setTangentHeadingInterpolation().build();

        outtake1 = follower.pathBuilder().addPath(
                new BezierLine(new Pose(126, 59.44), new Pose(84.000, 84.000))
        ...
        gate1 = follower.pathBuilder()
                ...
                .setConstantHeadingInterpolation(Math.toRadians(14.9))
                .build();

        outtake2 = follower.pathBuilder().addPath(
                new BezierLine(new Pose(136.87, 57.25), new Pose(84.000, 84.000))
        ).setTangentHeadingInterpolation().setReversed().build();
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            robot.turret.loopAuto(false, follower.getPose(), 146, 142);
